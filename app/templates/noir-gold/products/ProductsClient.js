    ...
    <main className={s.page}>
      <header className={s.header}>
        <nav className={`${s.wrap} ${s.nav}`}>
          <Link href="/templates/noir-gold" className={s.logo}>
            MVIA<span>.</span>
          </Link>
          <div className={s.navlinks}>
            <Link href="/templates/noir-gold#method">Method</Link>
            <Link href="/templates/noir-gold#programs">Programs</Link>
            <Link href="/templates/noir-gold#about">About</Link>
            <Link href="/templates/noir-gold#contact">Contact</Link>
          </div>
            ...
            {filtered.map((product, i) => (
              <div className={s.card} style={{ animationDelay: `${i * 0.05}s` }} key={product.id}>
                <div
                  className={s.thumb}
                  style={{
                    background: `linear-gradient(150deg, #17140F, #241d12)`,
                  }}
                >
      ...
      <footer className={s.footer}>
        &copy; 2026 THE MVIA INC. All rights reserved. &middot; Product demo &mdash; Noir &amp; Gold
      </footer>
    </main>
  );
}
